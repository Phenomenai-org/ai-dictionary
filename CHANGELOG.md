...
- **Model Stats View** — togglable per-model analytics on the `/stats/` page: model selector with summary cards, donut pie charts for all-ratings and latest-score distributions, stacked horizontal bar chart of per-term score breakdowns with a min-evaluations filter, and an N×N pairwise congruence matrix with selectable models and color-coded cells; backed by a new `term_scores` export in `models.json`
